...
//    if (!card_initialized) return RES_NOTRDY;
    	...
    	lcd_clear();
        lcd_put_cur(0,0);
        lcd_send_string("disk_read"); //6
        HAL_Delay(500);

        if (pdrv != 0 || count == 0) {
        	lcd_clear();
        	lcd_put_cur(0,0);
        	lcd_send_string("PARERR");
        	HAL_Delay(500);
            return RES_PARERR;
        }
        ...
        lcd_clear();
        lcd_put_cur(0,0);
        lcd_send_string("call RBlk"); //7
        HAL_Delay(500);

        DRESULT res = (SD_ReadBlocks(buff, sector, count) == SD_OK) ? RES_OK : RES_ERROR;
        char buf[10];
        sprintf(buf, "%d", res);

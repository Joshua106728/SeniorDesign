...
static void SD_ReceiveBuffer(uint8_t *buffer, uint16_t len) {
    ...
    dma_rx_done = 0;
    HAL_SPI_TransmitReceive_DMA(&hspi3, tx_dummy, buffer, len);
    while (!dma_rx_done);
#else
...
//            addr = sector;          // SDHC
//        } else {
//            addr = sector * 512;    // SDSC
//        }
//
//        SD_SendCommand(CMD17, addr, 0xFF);

        uint8_t cmd_resp = SD_SendCommand(CMD17, sector, 0xFF);
        if (cmd_resp != 0x00) {
        	char buf[10];
        	sprintf(buf, "%d", cmd_resp);
